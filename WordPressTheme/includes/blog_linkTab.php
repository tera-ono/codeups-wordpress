<!-- archive-blog.phpとtaxonomy-blog_category.phpの内容は同じにしたいので、条件分岐で出力するタブを制御するしテンプレート化する -->
<div class="c-termLinks">
  <?php if (is_post_type_archive('blog')) : ?>
    <span class="c-termLinks__items c-termLink is-active">すべて</span>
  <?php else : ?>
    <a href="<?php echo esc_url(get_post_type_archive_link('blog')); ?>" class="c-termLinks__items c-termLink">すべて</a>
  <?php endif; ?>
  <!-- 1.get_queried_object()で、現在クエリされているオブジェクトを取得 -->
  <!-- 2.get_terms()で、指定されたタクソノミー 制作ジャンル（blog_category）に含まれるターム（分類のオブジェクト）$genre_termsを取得します。
...
0 (false) （空のタームを出力する）-->
  ...
  ?>
    ...
    <?php if (is_tax('blog_category') && $queried_object->slug == $genre_term->slug) : ?>
    ...
    <?php else : ?>
      <a href="<?php echo esc_url(get_term_link($genre_term, 'blog_category')); ?>" class="c-termLinks__items c-termLink"><?php echo esc_html($genre_term->name); ?>
      </a>
    <?php endif; ?>
  <?php endforeach; ?>
</div>
